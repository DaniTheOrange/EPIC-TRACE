...
parser.add_argument("--skip",default=False,action="store_true",	help="Use skip connections from input to chain concatenation after transformer/self-attention.") # removed smoothly
parser.add_argument("-f","--conv_2_filter_size",type=int,default=0,help="Filter size for the second convolutional layer. If 0, no second convolutional layer will be used.") # removed smoothly
parser.add_argument("--num_tf_blocks",type=int,default=0,help="Number of transformer blocks to use. If 0, no transformer blocks will be used only Self-Attention without the feed forward parts.") # removed smoothly
parser.add_argument("--num_heads",type=int,default =5,help="Number of heads for the self attention layers.") # removed smoothly
parser.add_argument("--output_seq_len",action="store_true", help="Dummy argument to be compatible with saved models") # removed smoothly 
parser.add_argument("--test_task",type=int,default= -1,help="The task index i.e. 1 TPP2 and 2 for TPP3 for automatic (micro) AUROC and AP testing for file '<--dataset>_tpp<2/3>_data' + extension= '.gz' / '.csv'. If -1, no automatic testing will be done.") # removed smoothly
parser.add_argument("--feed_forward",action="store_true",default=False,help="Use feed forward in addition to convolution from input embeddings") # removed smoothly
parser.add_argument("--lr_find",action="store_true",default=False,help="Use pytorch lighting learning rate finder to find optimal learning rate")	# removed smoothly
parser.add_argument("--es_epoch_3",action="store_true",default=False,help="Use early stopping after 3 epochs instead of 8 iterations") # removed smoothly
...
parser.add_argument("--wd",type=float,default=1e-2,help="Weight decay for the optimizer.")  #check usage
